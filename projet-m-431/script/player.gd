...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("jump") and is_on_floor() and not is_sliding and not is_attacking and not is_drinking:
	...
	if Input.is_action_just_pressed("slide") and not is_sliding and not is_attacking and not is_drinking and can_slide:
		if not is_on_floor():
			can_slide = false
		is_sliding = true
	...
	if is_sliding:
		...
		if direction != 0:
			...
			if not is_attacking and not is_drinking:
